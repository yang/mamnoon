...
//			);
		...
		_frame: function(frame) {
			...
			covers.removeClass('current').each(function(index, cover) {
				...
				transform = 'scale(' + 1 + ',' + scale + ') rotateY(' + angle + 'deg)';
		...
		refresh: function(duration, index) {	
			...
			that.element.stop().animate({
				...
				'complete':		function() {				
					...
					// console.log(this.childNodes[that.options.index].getAttribute('data'));
					// console.log(that.options.index);



					// console.log(document.getElementById('title'));
					// document.getElementById('title').innerHTML = this.childNodes[that.options.index].getAttribute('data');
