...
<!-- BEGINNING OF ORIGINAL -->
 ...
 <template v-if="!valid">
<template v-for="item in upserveSections">
  <template v-if="noFiltering">{{ item.name }}</template>
  <template v-else>
...
<template v-else>
  ...
  <template v-else>
    <template v-if="currentlyAvailable(item.timing_mask.start_time, item.timing_mask.end_time, item.timing_mask.rules, selectedDate, selectedTime)">{{ item.name }}</template>
  </template>
</template>
            </template>
<!-- END OF ORIGINAL -->
