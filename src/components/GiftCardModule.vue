...
export default {
  ...
  methods: {
    async lookupBalance() {
      let giftcardLookup = await this.$http.post("http://localhost:4000/user/lookupgiftcard", {
    ...
    useBalance() {
      ...
      this.$http
        .post("http://localhost:4000/user/lookupgiftcard", {
          cardNumber: this.cardNumberInput,
        })
        .then(function (response) {
          if (
            ...
            self.$http
              .post("http://localhost:4000/user/usegiftcard", {
